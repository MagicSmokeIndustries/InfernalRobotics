...
{
	public class ModuleIRServo : PartModule, IServo, IRescalable, IJointLockState
	{
		public bool InterpolatorActive { get { return Interpolator.Active; } }
		...
		public void DoTransformStuff(Transform trf)
		{
			...
			if(IsRotational)
			{
				trf.rotation = Quaternion.LookRotation(FixedMeshTransform.TransformDirection(-rotateAxis), 
																	 FixedMeshTransform.TransformDirection(zeroUp));
			}
			else
			{
				trf.rotation = Quaternion.LookRotation(FixedMeshTransform.TransformDirection(-translateAxis), 
																 FixedMeshTransform.TransformDirection(zeroUp));
			}
		}
		...
		public float Speed
		{
			get { return Interpolator.Velocity; }
		}

		public float DefaultSpeed
		{
			get { return rotateJoint ? keyRotateSpeed : keyTranslateSpeed; }
			set {}
		}

		public void MoveTo(float position, float speed) // normal
		{
			if(speed == 0.0f)
				speed = customSpeed * speedTweak;

			//because Translator expects position in external coordinates
			position = Translator.ToExternalPos(position);
			Translator.Move(position, customSpeed * speedTweak);
		}

		public void EditorMoveTo(float d)
		{}

		public void EditorSetTo(float position) // Editor
		{
			var deltaPosition = position - Position;
			EditorApplyDeltaPos(deltaPosition);
		}

		/*private readonly*/ public IMotor motor;
		/*private readonly*/ public IPresets presets;
		...
		public bool CanHaveLimits { get { return limitTweakable; } }
		...
		*/
		...
		public bool IsRotational { get { return rotateJoint; } }
		...
		{
			get { return customSpeed * speedTweak; }
			set { speedTweak = value / customSpeed; } // FEHLER, voll komischer Müll hier
		}

		[KSPField(isPersistant = true)] public bool invertAxis;
		[KSPField(isPersistant = true)] public float torqueMax = 30f;

		public bool IsInverted { get { return invertAxis; } set { invertAxis = value; } }
		public void ToggleInverted() { InvertAxisToggle(); }


		public float MaxTorque { get { return torqueMax; } }
	
		public float TorqueLimit
		{
			get { return torqueTweak * torqueMax; }
			set { torqueTweak = Mathf.Clamp(value, 0.1f, torqueMax) / torqueMax; }
		}
		...
		{
			get { return Translator.GetSpeedUnit(); }
		}

		public float AccelerationLimit
		{
			get { return accelTweak * Translator.GetSpeedUnit(); }
			set { accelTweak = Mathf.Clamp(value, 0.1f, Translator.GetSpeedUnit()) / Translator.GetSpeedUnit(); }
		}
		...
		public float InternalPosition { get { return Translator.ToInternalPos(Position); } }
			// FEHLER, Obermüll wegen dem wie's früher lief

		public float ExternalPosition { get { return Translator.ToExternalPos(Position); } }
			// FEHLER, Obermüll wegen dem wie's früher lief
		...
		public bool IsLimitted { get { return limitTweakableFlag; } }

		[KSPEvent(guiActive = true, guiActiveEditor = true, guiName = "Engage Limits", active = false)]
		public void ToggleLimits() // hiess früher LimitTweakableToggle
		{
			if(!rotateJoint)
				return;
			limitTweakableFlag = !limitTweakableFlag;
			Events["ToggleLimits"].guiName = limitTweakableFlag ? "Disengage Limits" : "Engage Limits";
		...
		public void InitUITweakables()
		{
			...
			try
			{
				...
				if(rotateJoint)
				{
					...
					if(limitTweakable)
					{
						Events["ToggleLimits"].active = true;
						Events["ToggleLimits"].guiName = limitTweakableFlag ? "Disengage Limits" : "Engage Limits";
					}
				...
				else if(translateJoint)
				{
					...
					Events["ToggleLimits"].active = false;
		...
		public override void OnAwake()
		{
			...
			Translator.Init(isMotionLock, this, Interpolator);
		...
		public override void OnStart(StartState state)
		{
			...
			if(limitTweakable)
			{
				Events["ToggleLimits"].active = rotateJoint;
				Events["ToggleLimits"].guiName = limitTweakableFlag ? "Disengage Limits" : "Engage Limits";
			}
...
// FEHLER, ab hier Zeugs, was aus dem Servo kommt und langsam hier reingebaut wird...
		...
		public void ApplyLimitsToSymmetry()	// FEHLER, wozu???
		{
			foreach(Part counterPart in part.symmetryCounterparts)
			{
				var module = ((ModuleIRServo)counterPart.Modules["ModuleIRServo"]);
			...
			Logger.Log ("ApplyingSymmetry, number of counterparts: " + part.symmetryCounterparts.Count, Logger.Level.Debug);
		}

		public void Reset()	// FEHLER, wozu???
		{
			if(HighLogic.LoadedSceneIsEditor)
			{
				if(rotateJoint)
				...
				{
					FixedMeshTransform.position = part.transform.position;
					translation = 0;
				}
			}
		}

		public void Reconfigure()	// FEHLER, wozu???
		{
			ConfigureInterpolator();
			SetupJoints ();
		}
		...
		{
			get { return rotateJoint ? rotateMin : translateMin; }
		}

		public float MaxPosition
		{
			get { return rotateJoint ? rotateMax : translateMax; }
		}
		...
		public float MinPositionLimit
		{
			...
			{
				if(rotateJoint)
					minTweak = Mathf.Clamp(value, rotateMin, rotateMax);
				else
					minTweak = Mathf.Clamp(value, translateMin, translateMax);
			}
		}
		...
		public float MaxPositionLimit
		{
			...
			{
				if(rotateJoint)
					maxTweak = Mathf.Clamp(value, rotateMin, rotateMax);
				else
					maxTweak = Mathf.Clamp(value, translateMin, translateMax);
			}
		}
		...
		{
			get { return Translator.ToExternalPos(defaultPosition); }
			set { defaultPosition = Math.Min(Math.Max(Translator.ToInternalPos(value), minTweak), maxTweak); }
		}

		public float InternalDefaultPosition { get { return Translator.ToInternalPos(DefaultPosition); } }
			// FEHLER, Obermüll wegen dem wie's früher lief

		public float ExternalDefaultPosition { get { return Translator.ToExternalPos(DefaultPosition); } }
			// FEHLER, Obermüll wegen dem wie's früher lief


		public bool IsMoving
		{
			get { return Translator.IsMoving(); }
		}

		public bool IsFreeMoving
		{
			get { return freeMoving; }
		}

		public bool IsLocked
		{
			get { return Translator.IsMotionLock; }
			set { SetLock(value); }
		}
/*
...
*/
		public float ElectricChargeRequired
		{
			...
			set { electricChargeRequired = value; }
		}
		...
		// dieses Zeug hier dient nur noch als "zeig auf" Ziel :-) ... tja hey... was soll man machen? ...

		public static Transform PointTo = null;

		[KSPEvent(guiActive = true, guiName = "zeig auf")]
		public void ZeigAuf()
		{
			if(PointTo == part.transform)
				PointTo = null;
			else
				PointTo = part.transform;
		}
	}
}